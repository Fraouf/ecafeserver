class CustomersController < ApplicationController
  ...
  def show
    @customer = Customer.find_by_uid(params[:id])
    ...
    if @model
      ...
      if @customer
        ...
        if @timecode.save
          ...
          redirect_to :controller => "customers", :action => "show", :id => @customer.uid
        else
          flash[:error] = t 'timecodes.add_failed'
          redirect_to :controller => "customers", :action => "new_credit", :id => @customer.uid
        end
      end
		end
  end
